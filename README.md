...
# OGM updates can be really confusing

OGM's "save" method takes some getting used to, and if you don't use it properly it can easily generate and run queries that are successful but don't do what you expect.
...
The second big confusion with OGM saving is when your objects have circular references, because the comparison OGM does is based on the full object graph it's seeing in the save method. In the example of trying to update a Team's captain, if you have circular references in your model objects such that a Team has reference to a captain which has reference back to its Team, it's not enough to load the Team, replace its captain reference, and then save the Team, because the fact that the captain still has the backreference back to the original Team means OGM won't consider the original relationship gone and will once again just add the new one, leaving you with a Team with two captains. To do the update correctly you need to carefully change the references in both sides of the cycle - set the Team's captain field and ALSO set the captain's team field, so that the object graph OGM sees no longer has that old relationship at all.
